...
class Configurator {
  constructor (container, items, options) {
    ...
    const keys = Object.keys(window.com.greensock.easing)
    const easings = new Map([])
    const easingLookup = new Map([])
    keys.forEach(easeName => {
      const easeMeta = { name: easeName, types: [] }
      const ease = window.com.greensock.easing[easeName]
      const validTypes = ['easeIn', 'easeOut', 'easeInOut', 'easeNone']
      validTypes.forEach(type => {
        const easeType = ease[type]
        if (easeType) {
  ...
  onEffectIdGUIFinishChange (id) {
    ...
    const toGridEaseTypes = this.easings.get(toGridEasingMeta.name)
    this.setControllerOptions(
      this.guiControllers.toGridEaseType,
      toGridEaseTypes,
      true
    )
    const toFullEaseTypes = this.easings.get(toFullEasingMeta.name)
    this.setControllerOptions(
  ...
  init () {
    ...
    const progressGUI = gui
      ...
      .onChange(progress => {
        ...
        if (this.onProgressChange) { this.onProgressChange({ index: effect.currentImageIndex, progress }) }
        if (progress > 0) {
    ...
    this.guiControllers.toGridEaseName = easingFolder
      ...
      .name('To-Grid easing')
      .onFinishChange(name => {
        const easeTypes = this.easings.get(name)
        this.setControllerOptions(
          this.guiControllers.toGridEaseType,
          easeTypes
        )
        ...
        case 'activation':
          ...
          if (propsArray.length > 0) {
            ...
            for (let i = 0; i < propsArray.length; i += 2) {
              ...
              if (propValue.length > 0 && !isNaN(propValue * 1)) { propValue = propValue * 1 } else if (propValue === 'true' || propValue === 'false') { propValue = propValue === 'true' }
